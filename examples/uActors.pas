...
begin
  ...
  FPos.Assign( Topaz.Math.RandomRange(0, Topaz.Window.Width-1), Topaz.Math.RandomRange(0, Topaz.Window.Height-1));

  FRange.MinX := 0;
  FRange.MinY := 0;

  FSize := Topaz.Math.RandomRange(25, 100);

  FRange.MaxX := (Topaz.Window.Width-1) - FSize;
  FRange.MaxY := (Topaz.Window.Height-1) - FSize;

  FSpeed.x := Topaz.Math.RandomRange(120, 120*3);
  FSpeed.y := Topaz.Math.RandomRange(120, 120*3);

  LR := Topaz.Math.RandomRange(1, 255);
  LG := Topaz.Math.RandomRange(1, 255);
  LB := Topaz.Math.RandomRange(1, 255);
  FColor := Topaz.Color.FromByte(LR,LG,LB,255);
end;
...
begin
  Topaz.Window.DrawFilledRectangle(FPos.X, FPos.Y, FSize, FSize, FColor);
end;

{ TActorEx }
procedure TActorEx.OnGetSettings(var aSettings: TGameSettings);
begin
  inherited;
  ...
  // Window
  aSettings.WindowTitle := aSettings.WindowTitle + ' - Actor Example';
end;
...
procedure TActorEx.OnRenderHUD;
begin
  inherited;

  HudText(Font, GREEN,  haLeft, HudTextItem('S', 'Spawn actors'), []);
  HudText(Font, YELLOW, haLeft, HudTextItem('Count', '%d', ''), [Scene.GetList(0).GetCount]);
end;
...
begin
  Scene.ClearAll;
  LCount := Topaz.Math.RandomRange(3, 25);
  for I := 1 to LCount do
    Scene.GetList(0).Add(TMyActor.Create);
end;
