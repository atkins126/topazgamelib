...
type
  ...
  { TPolyPointCollisionEx }
  TPolyPointCollisionEx = class(TGame)
  protected
    FBoss: IEntity;
    FFigure: IEntity;
    FHitPos: TVector;
...
{ TExampleTemplate }
  TPolyPointCollisionPointEx = class(TGame)
  protected
    FFigure: IEntity;
    FFigureAngle: Single;
    FCollide: Boolean;
...
{ TEntity }
procedure TEntityEx.OnGetSettings(var aSettings: TGameSettings);
begin
  inherited;

  aSettings.ArchiveFilename := 'Data.arc';

  aSettings.WindowTitle := aSettings.WindowTitle + ' - Entity Examples';
end;
...
begin
  ...
  FEntity.SetPosAbs(Topaz.Window.Width/2, Topaz.Window.Height/2);
...
{ TPolyPointCollisionEx }
procedure TPolyPointCollisionEx.OnGetSettings(var aSettings: TGameSettings);
begin
  inherited;
  ...
  // Window
  aSettings.WindowTitle := aSettings.WindowTitle + ' - Entity PolyPoint Collision Example';
end;

procedure TPolyPointCollisionEx.OnStartup;
begin
  inherited;
  ...
  // init boss entity
  Topaz.Get(IEntity, FBoss);
  FBoss.Init(Sprite, 0);
  FBoss.SetFrameFPS(14);
  FBoss.SetScaleAbs(1);
  FBoss.SetPosAbs(Topaz.Window.Width/2, (Topaz.Window.Height/2)-100);
  FBoss.TracePolyPoint(6, 12, 70);
  FBoss.SetRenderPolyPoint(True);

  // init figure entity
  Topaz.Get(IEntity, FFigure);
  FFigure.Init(Sprite, 1);
  FFigure.SetFrameFPS(17);
  FFigure.SetScaleAbs(1);
  FFigure.SetPosAbs(Topaz.Window.Width/2, Topaz.Window.Height/2);
  FFigure.TracePolyPoint(6, 12, 70);
  FFigure.SetRenderPolyPoint(True);

end;

procedure TPolyPointCollisionEx.OnShutdown;
begin
  Topaz.Release(FFigure);
  Topaz.Release(FBoss);

  inherited;
end;

procedure TPolyPointCollisionEx.OnUpdate(aDeltaTime: Double);
begin
  inherited;

  FBoss.NextFrame;
  FBoss.ThrustToPos(30*50, 14*50, GetMousePos.X, GetMousePos.Y, 128, 32, 5*50, EPSILON, aDeltaTime);
  ...
  Topaz.Math.ClipValueFloat(FFigureAngle, 0, 359, True);
  FFigure.RotateAbs(FFigureAngle);
end;


procedure TPolyPointCollisionEx.OnRender;
begin
  inherited;
  ...
  FBoss.Render(0, 0);
  if FCollide then
    Topaz.Window.DrawFilledRectangle(FHitPos.X, FHitPos.Y, 10, 10, RED);
end;


{ TEntityPolyPointCollisionPoint }
procedure TPolyPointCollisionPointEx.OnGetSettings(var aSettings: TGameSettings);
begin
  inherited;

  aSettings.ArchiveFilename := 'Data.arc';

  aSettings.WindowTitle := aSettings.WindowTitle + ' - Entity PolyPoint Collision Point';
end;

procedure TPolyPointCollisionPointEx.OnStartup;
begin
  inherited;
...
procedure TPolyPointCollisionPointEx.OnShutdown;
begin
  Topaz.Release(FFigure);
  inherited;
end;

procedure TPolyPointCollisionPointEx.OnUpdate(aDeltaTime: Double);
begin
  inherited;

  FHitPos.Assign(GetMousePos^);
...
procedure TPolyPointCollisionPointEx.OnRender;
begin
  inherited;
...
procedure TPolyPointCollisionPointEx.OnRenderHUD;
begin
  inherited;

  Font.PrintText(Topaz.Window.Width/2, (Topaz.Window.Height/2) - 100, YELLOW, haCenter, 'Move mouse pointer over figure outline', []);
end;
