...
{ TExampleTemplate }
procedure TAsyncConsoleEx.OnInit;
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnDone;
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnRun;
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnGetSettings(var aSettings: TGameSettings);
begin
  inherited;
  ...
  // Window
  aSettings.WindowTitle := aSettings.WindowTitle + ' - Console Async Example';
  aSettings.WindowClearColor := BLACK;
end;

procedure TAsyncConsoleEx.OnStartup;
begin
  inherited;
...
procedure TAsyncConsoleEx.OnShutdown;
begin
...
procedure TAsyncConsoleEx.OnReady(aReady: Boolean);
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnUpdate(aDeltaTime: Double);
begin
  inherited;

  if Topaz.Input.KeyPressed(KEY_A) then
  begin
    Topaz.Async.Run(
    'TAsyncConsole',
    procedure
    ...
    begin
      for I := 1 to 1000 do
      begin
        Topaz.Console.PrintLn('%d', [I]);
      end;
    end,
    procedure
    begin
      Topaz.Console.PrintLn('Done!');
    end
    );
  end;
...
procedure TAsyncConsoleEx.OnFixedUpdate;
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnClearWindow;
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnShowWindow;
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnRender;
begin
  inherited;
...
procedure TAsyncConsoleEx.OnRenderHUD;
begin
  inherited;

  HudText(Font, GREEN,  haLeft, HudTextItem('A', 'Start async task'), []);
  HudText(Font, ORANGE, haLeft, HudTextItem('Busy', '%s'), [FALSE_TRUE_STR[Topaz.Async.Busy('TAsyncConsole')]]);

end;

procedure TAsyncConsoleEx.OnPreShowWindow;
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnPostShowWindow;
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnCmdConsoleState(aState: TCmdConsoleState);
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnVideoState(aState: TVideoState; aFilename: WideString);
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnScreenshot(const aFilename: WideString);
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnProcessIMGUI;
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnDisposeActor(aActor: TActor);
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnSetupStartupDialog;
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnStartupDialogMore;
begin
  inherited;
end;

function  TAsyncConsoleEx.OnStartupDialogRun: Boolean;
begin
  Result := inherited;
end;

procedure TAsyncConsoleEx.OnBuildArchiveProgress(const aFilename: WideString; aProgress: Integer; aNewFile: Boolean);
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnSpeechWord(const aWord, aText: WideString);
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnHighscoreAction(aHighscores: IHighscores; aAction: THighscoreAction);
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnInAppPurchase(aIAP: IInAppPurchase);
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnSocialPost(const aSuccess: Boolean; const aErrorMsg, aMsg, aMediaFilename: WideString);
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnSendMail(const aFromEmail, aFromName, aSubject, aToEmail, aBody, aError: WideString);
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnLuaState(aState: TLuaState);
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnBeforeRenderScene(aSceneNum: Integer);
begin
  inherited;
end;

procedure TAsyncConsoleEx.OnAfterRenderScene(aSceneNum: Integer);
begin
  inherited;
end;
