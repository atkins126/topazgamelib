...
begin
  ...
  // Window
  aSettings.WindowTitle := aSettings.WindowTitle + ' - GUI Example';
  aSettings.WindowClearColor := BLACK;
end;

procedure TGUIEx.OnStartup;
begin
  inherited;
  ...
  FSfx := Topaz.Audio.LoadSound(Archive, 'arc/sfx/digthis.ogg');

  FMusic := Topaz.Audio.LoadMusic(Archive, 'arc/music/song07.ogg');
  Topaz.Audio.PlayMusic(FMusic, MusicVolume, True);

end;

procedure TGUIEx.OnShutdown;
begin
...
begin
  ...
  if Topaz.GUI.WindowBegin('Window 1', 'Window 1', 50, 50, 270, 220, cGuiWindowFlags) then
  begin
    ...
    if Topaz.GUI.Option('hard', Boolean(Difficulty = 1)) then
      Difficulty := 1;

    Topaz.GUI.LayoutRowBegin(GUI_STATIC, 30, 2);
    Topaz.GUI.LayoutRowPush(50);
    Topaz.GUI.&Label('Volume:', GUI_TEXT_LEFT);
    Topaz.GUI.LayoutRowPush(110);
    if Topaz.GUI.Slider(0, 1, 0.01, MusicVolume) then
      Topaz.Audio.SetMusicVolume(FMusic, MusicVolume);
    Topaz.GUI.LayoutRowPush(120);
    if Topaz.GUI.Checkbox('Dig this', chk1) then
    begin
      if chk1 then
      begin
        Topaz.Audio.PlaySound(AUDIO_DYNAMIC_CHANNEL, FSfx, 0.5, False);
      end;
    end;
    Topaz.GUI.Checkbox('Change theme', chk2);
    Topaz.GUI.LayoutRowEnd;
  end;
  Topaz.GUI.WindowEnd;

  if chk2 then
  begin
    if Topaz.GUI.WindowBegin('Window 2', 'Window 2', 350, 150, 320, 220, cGuiWindowFlags) then
      begin
       Topaz.GUI.LayoutRowStatic(25, 190, 1);
       Theme := Topaz.GUI.Combobox(cGuiThemes, Theme, 25, 200, 200, ThemeChanged);
      end
    else
      begin
       chk2 := False;
