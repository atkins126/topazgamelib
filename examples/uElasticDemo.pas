...
{ TElasticDemo }
procedure TElasticDemo.OnGetSettings(var aSettings: TGameSettings);
begin
  inherited;

  aSettings.ArchiveFilename := 'Data.arc';

  aSettings.WindowTitle := aSettings.WindowTitle + ' - Elastic Demo';
...
begin
  ...
  FMusic := Topaz.Audio.LoadMusic(Archive, 'arc/music/song04.ogg');
...
begin
  ...
  if not Topaz.Timer.FrameSpeed(FTimer, Topaz.Timer.GetUpdateSpeed) then
    Exit;

  FBead[0].X := GetMousePos.X;
  FBead[0].Y := GetMousePos.Y;
...
begin
  ...
  // draw last bead
  Topaz.Window.DrawFilledRectangle(FBead[0].X, FBead[0].Y, cBeadSize, cBeadSize, GREEN);

  // loop though other beads
  for I := 1 to cBeadCount do
  begin
    // draw bead and string from it to the one before it
    Topaz.Window.DrawLine(FBead[I].x+cBedHalfSize,
      FBead[I].y+cBedHalfSize, FBead[I-1].x+cBedHalfSize,
      FBead[I-1].y+cBedHalfSize, 1, YELLOW);
    Topaz.Window.DrawFilledRectangle(FBead[I].X, FBead[I].Y, cBeadSize, cBeadSize, GREEN);
  end;
end;
