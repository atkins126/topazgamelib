...
{ TShaderEx }
procedure TShaderEx.OnGetSettings(var aSettings: TGameSettings);
begin
  inherited;
  ...
  // Window
  aSettings.WindowTitle := aSettings.WindowTitle + ' - Shader Example';
...
begin
  ...
  for I := 0 to 2 do
  begin
    Topaz.Get(IShader, FShader[I]);
    FShader[I].Load(Archive, stFragment, Topaz.Util.Format('arc/shaders/%s.frag', [CName[I]]));
    FShader[I].Build;

    FShader[I].Enable(True);
    FShader[I].SetVec2Uniform('u_resolution', Topaz.Window.Width * Topaz.Window.Scale, Topaz.Window.Height * Topaz.Window.Scale);
...
begin
  ...
  FShader[FIndex].SetFloatUniform('u_time', Topaz.Util.GetTime);
...
begin
  ...
  FShader[FIndex].Enable(True);
  Topaz.Window.DrawFilledRectangle(0, 0, Topaz.Window.Width, Topaz.Window.Height, WHITE);
  FShader[FIndex].Enable(False);
end;
