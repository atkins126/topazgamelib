...
type
  ...
  { TRotateRenderTargetEx }
  TRotateRenderTargetEx = class(TGame)
  protected
    FRenderTarget: IRenderTarget;
    FBackground: ITexture;
    FSpeed: Single;
    FAngle: Single;
  public
...
{ TExampleTemplate }
procedure TRotateRenderTargetEx.OnInit;
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnDone;
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnRun;
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnGetSettings(var aSettings: TGameSettings);
begin
  inherited;
  ...
  // Window
  aSettings.WindowTitle := aSettings.WindowTitle + ' - Rotate RenderTarget Example';
end;

procedure TRotateRenderTargetEx.OnStartup;
begin
  inherited;
  ...
  Topaz.Get(IRenderTarget, FRenderTarget);
  FRenderTarget.Init((Topaz.Window.Width - 380) div 2, (Topaz.Window.Height - 280) div 2, 380, 280);

  Topaz.Get(ITexture, FBackground);
  FBackground.Load(Archive, 'arc/images/bluestone.png', nil);
end;

procedure TRotateRenderTargetEx.OnShutdown;
begin
  Topaz.Release(FBackground);
  Topaz.Release(FRenderTarget);

  inherited;
end;

procedure TRotateRenderTargetEx.OnReady(aReady: Boolean);
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnUpdate(aDeltaTime: Double);
begin
  inherited;

  FSpeed := FSpeed + (60 * aDeltaTime);
  FAngle := FAngle + (7 * aDeltaTime);
  Topaz.Math.ClipValueFloat(FAngle, 0, 359, True);
  FRenderTarget.SetAngle(FAngle);
end;

procedure TRotateRenderTargetEx.OnFixedUpdate;
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnClearWindow;
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnShowWindow;
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnRender;
var
  LSize: TRectangle;
begin
  inherited;
...
procedure TRotateRenderTargetEx.OnRenderHUD;
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnPreShowWindow;
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnPostShowWindow;
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnCmdConsoleState(aState: TCmdConsoleState);
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnVideoState(aState: TVideoState; aFilename: WideString);
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnScreenshot(const aFilename: WideString);
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnProcessIMGUI;
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnDisposeActor(aActor: TActor);
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnSetupStartupDialog;
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnStartupDialogMore;
begin
  inherited;
end;

function  TRotateRenderTargetEx.OnStartupDialogRun: Boolean;
begin
  Result := inherited;
end;

procedure TRotateRenderTargetEx.OnBuildArchiveProgress(const aFilename: WideString; aProgress: Integer; aNewFile: Boolean);
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnSpeechWord(const aWord, aText: WideString);
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnHighscoreAction(aHighscores: IHighscores; aAction: THighscoreAction);
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnInAppPurchase(aIAP: IInAppPurchase);
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnSocialPost(const aSuccess: Boolean; const aErrorMsg, aMsg, aMediaFilename: WideString);
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnSendMail(const aFromEmail, aFromName, aSubject, aToEmail, aBody, aError: WideString);
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnLuaState(aState: TLuaState);
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnBeforeRenderScene(aSceneNum: Integer);
begin
  inherited;
end;

procedure TRotateRenderTargetEx.OnAfterRenderScene(aSceneNum: Integer);
begin
  inherited;
end;
